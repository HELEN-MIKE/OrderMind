...
- 支持 TXT、CSV、TSV、XLSX、XLSM、文本型 PDF 文件导入。
...
- 审单结果页支持导出本地 HTML 和 Excel 报告，方便客户保存、筛选或转发试用结果。
- 提供命令行入口，方便批量审单和开发验证。
...
python3 -m ordermind.cli samples/customer_like_orders/text_pdf_order.pdf --template templates/default_order_rules.json
```
...
```text
samples/customer_like_orders/
```

这些样例全部使用虚构公司名、联系人、地址和订单号，不包含真实客户或供应商信息。它们覆盖中文采购订单、英文商业发票、英文形式发票、Excel 多币种订单备注、文本型 PDF 订单，以及一份故意带错误的审单演示样例。

可直接用 CLI 验证：

```bash
python3 -m ordermind.cli samples/customer_like_orders/domestic_purchase_order_zh.txt --template templates/default_order_rules.json
...
python3 -m ordermind.cli samples/customer_like_orders/multi_currency_order.xlsx --template templates/default_order_rules.json
python3 -m ordermind.cli samples/customer_like_orders/text_pdf_order.pdf --template templates/default_order_rules.json
```

故意带问题的样例会返回非 0 状态，用于演示问题发现：

```bash
python3 -m ordermind.cli samples/customer_like_orders/review_findings_bad_amount_missing_material.txt --template templates/default_order_rules.json
```
...
第一版是可运行 MVP，不是最终文档智能引擎。当前 PDF 支持范围是可复制文本的电子 PDF；扫描件 OCR、复杂 PDF 表格恢复、DOC、DOCX、复杂合并单元格、跨页表格、本地大模型语义识别会在后续版本扩展。
